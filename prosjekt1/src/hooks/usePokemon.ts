...
const fetchPokemon = async (nameOrId: string | number) => {
    const cachedData = localStorage.getItem(`pokemon-${nameOrId}`);
    if (cachedData) {
        try {
            return JSON.parse(cachedData);
        } catch (error) {
            console.error('Failed to parse cached Pokémon data:', error);
            localStorage.removeItem(`pokemon-${nameOrId}`);
        }
    }
    ...
    try {
        localStorage.setItem(`pokemon-${nameOrId}`, JSON.stringify(minimalData));
    } catch (error) {
